...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      ...
      constant->constAttrs->value  = constantValue;
...
void compileBlock2(void) {
  ...
  if (lookAhead->tokenType == KW_TYPE) {
    ...
    do {
      ...
      typeValue = compileType();
      typeObj->typeAttrs->actualType = typeValue;
...
void compileFuncDecl(void) {
  ...
  funcType = compileBasicType();
  funcObj->funcAttrs->returnType = funcType;
...
ConstantValue* compileConstant(void) {
  ...
  case SB_MINUS:
    eat(SB_MINUS);
    constValue = compileConstant2();
    constValue->intValue = -constValue->intValue; //Negative int
    break;
  case TK_CHAR:
    eat(TK_CHAR);
    constValue = makeCharConstant(currentToken->string[0]);
